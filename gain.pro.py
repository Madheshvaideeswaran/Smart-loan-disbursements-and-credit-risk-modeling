...
for pkg in ['catboost', 'statsmodels', 'plotly', 'fpdf2', 'kaleido', 'openpyxl']:
    try:
        __import__(pkg)
    except ImportError:
        import subprocess
...
# =============================
# Tab 3: SARIMAX Forecast — CLOUD-SAFE GRAPH + SILENT EXPORT
# =============================
with tab3:
    ...
    # === Interactive Plotly Chart — CLOUD-SAFE VERSION ===
    fig = go.Figure()
    ...
    # ✅ CLOUD-SAFE LAYOUT — NO FONT FAMILY, USE WEIGHT
    fig.update_layout(
        title=dict(
            text="12-Month Disbursement Forecast: Baseline vs Adjusted Scenario",
            font=dict(size=18, color='black', weight='bold')  # ← SAFE
        ),
        xaxis=dict(
            title="Date",
            titlefont=dict(size=14, color='black', weight='bold'),  # ← SAFE
            tickfont=dict(size=13, color='black'),  # ← SAFE
            showgrid=True,
            gridcolor='lightgray',
            zeroline=False
        ),
        ...
        legend=dict(
            ...
            font=dict(size=13, color='black', weight='bold')  # ← SAFE
        ),
        hovermode='x unified',
    ...
    # === PDF Export — SILENT KALEIDO FALLBACK ===
