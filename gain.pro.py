...
for pkg in ['catboost', 'statsmodels', 'plotly', 'fpdf2', 'kaleido']:
    try:
        __import__(pkg)
    except ImportError:
        import subprocess
...
# =============================
# Tab 3: SARIMAX Forecast — ENHANCED GRAPH + SILENT KALEIDO FALLBACK
# =============================
with tab3:
    ...
    # Adjustment factor — UNCHANGED LOGIC (uses delta_risk_pct from relative change)
    SENSITIVITY = 0.8
    ...
    # === Interactive Plotly Chart with White Background + ENHANCED VISIBILITY ===
    fig = go.Figure()
    ...
    # Confidence Intervals — Filled areas
    fig.add_trace(go.Scatter(
    ...
    # Update layout for white background and MAXIMUM VISIBILITY
    fig.update_layout(
        title=dict(
    ...
    # === PDF Export with Chart Image — SILENT FAILURE (no warning) ===
    ...
    def create_pdf():
        ...
        # Add Chart Image — silently skip if fails
        chart_saved = False
        try:
            fig.write_image("temp_forecast.png", width=800, height=400, engine="kaleido")
            pdf.image("temp_forecast.png", x=10, y=pdf.get_y(), w=180, h=90)
            pdf.ln(10)
            chart_saved = True
        except:
            # ❌ DO NOT SHOW ANY WARNING — silent fallback
            pass
        ...
        # Cleanup temp file
        if chart_saved and os.path.exists("temp_forecast.png"):
            os.remove("temp_forecast.png")

        # Return as bytes — convert bytearray to bytes if needed
