...
with tab3:
    ...
    # === PDF Export — PERFECTLY ALIGNED, MULTI-PAGE ===
    st.subheader("Export Reports")

    def create_pdf():
        pdf = fpdf.FPDF(orientation='P', unit='mm', format='A4')
        pdf.set_auto_page_break(auto=False, margin=15)  # Manual control
        pdf.add_page()
        ...
        # === Helper: Safely add image with auto page break ===
        def safe_add_image(filepath, img_width, img_height):
            # Ensure image fits on current page (A4 height = 297mm, safe margin 15mm → 282mm max)
            if pdf.get_y() + img_height > 270:  # 270 = conservative safe zone
                pdf.add_page()
            pdf.image(filepath, x=15, y=pdf.get_y(), w=img_width, h=img_height)
            pdf.ln(img_height + 8)  # Space after image

        # === Generate & Embed Forecast Chart ===
        ...
        plt.close()
        safe_add_image("temp_forecast.png", 170, 85)
        ...
        plt.close()
        safe_add_image("temp_pie.png", 170, 70)

        # === Generate & Embed Confusion Matrix ===
        from sklearn.metrics import confusion_matrix
        cm = confusion_matrix(y_test, y_pred, labels=["Low Risk", "Medium Risk", "High Risk"])
        plt.figure(figsize=(6, 4))
        ...
        plt.close()
        safe_add_image("temp_cm.png", 170, 70)

        # === Add Table ===
        if pdf.get_y() > 250:  # If not enough space, new page
            pdf.add_page()
        ...
        for idx, row in impact_df.iterrows():
            ...
            pdf.cell(col_widths[1], 10, str(row["Baseline"]), border=1)
            pdf.cell(col_widths[2], 10, str(row["Adjusted Scenario"]), border=1)
            pdf.ln(10)
